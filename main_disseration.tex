...
The resources generated for each map are randomly placed, with random calorific values. 
...
to understanding the differences between the two algorithms. A visualisation of the map and life form
was developed early on. The visualisation was developed using the model view controller architecture,
which gave meant that the map and the life form were completely detached from the visualisation.
The map and life form are abstracted using an abstract class which contains the basic variables need for the life
