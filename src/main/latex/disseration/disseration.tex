...
Some simple methods for comparing the algorithms will be runtime and memory/space requirements of the evolutionary computation to compute a solution to a given problem.
...
random population members. The fittest members of this population are then selected to create a new generation through cross over and mutation. 
...
Generally for evolving good fit life forms an unique map is generated for each simulation. This creates an environment in the life form has not
...
This can however be a significant drain on computational power, and will slow down the simulation. This graph can be disabled to give
...
To help in the comparison of the two algorithms an user interface was developed. There are several cal parts to the user interface, the main
...
\section{Conclusion}
The project has generated interesting findings along with an useful tool for further information gathering. The finds relate to the implementation of each algorithm
